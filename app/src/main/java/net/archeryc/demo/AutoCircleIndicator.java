...
public class AutoCircleIndicator extends LinearLayout implements OnPageChangeListener {
    ...
    public AutoCircleIndicator(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    private void handleTypedArray(Context context, AttributeSet attrs) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.AutoCircleIndicator);
        mIndicatorWidth =
                typedArray.getDimensionPixelSize(R.styleable.AutoCircleIndicator_ci_width, -1);
        mIndicatorHeight =
                typedArray.getDimensionPixelSize(R.styleable.AutoCircleIndicator_ci_height, -1);
        mIndicatorMargin =
                typedArray.getDimensionPixelSize(R.styleable.AutoCircleIndicator_ci_margin, -1);

        mAnimatorResId = typedArray.getResourceId(R.styleable.AutoCircleIndicator_ci_animator,
                R.animator.scale_with_alpha);
        mAnimatorReverseResId =
                typedArray.getResourceId(R.styleable.AutoCircleIndicator_ci_animator_reverse, 0);
        mIndicatorBackgroundResId =
                typedArray.getResourceId(R.styleable.AutoCircleIndicator_ci_drawable,
                        R.drawable.white_radius);
        mIndicatorUnselectedBackgroundResId =
                typedArray.getResourceId(R.styleable.AutoCircleIndicator_ci_drawable_unselected,
                        mIndicatorBackgroundResId);
        typedArray.recycle();
    }
    ...
    public void onPageSelected(int position) {
        ...
        mCurrentPosition = position % mViewpager.getAutoAdapter().getRealCount();

        View selectedIndicator = getChildAt(mCurrentPosition);
    ...
    public void setViewPager(AutoViewPager autoViewPager) {
        ...
        mCurrentPosition = mViewpager.getAutoAdapter().getRealCurrentItem();
        createIndicators(autoViewPager);
        mViewpager.removeOnPageChangeListener(this);
        mViewpager.addOnPageChangeListener(this);
        onPageSelected(mCurrentPosition);
    }

    private void createIndicators(AutoViewPager viewPager) {
        removeAllViews();
        if (viewPager.getAdapter() == null) {
            return;
        }

        int count = viewPager.getAutoAdapter().getRealCount();
        if (count <= 0) {
            return;
        }
